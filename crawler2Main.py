...
def crawl_page(zipcode, page_num, verbose=False):
    ...
    restaurants = soup.findAll('div', attrs={'class':re.compile(r'^mainAttributes')})
    ...
    #result = restaurants[10:20]
    for r in restaurants[10:20]:
        print(r)
        yelpPage = ''
	title = ''
	restRating = ''
	try:
            title = r.find('a', {'class':'lemon--a__373c0__IEZFH link__373c0__29943 link-color--blue-dark__373c0__1mhJo link-size--inherit__373c0__2JXk5'}).getText().encode('utf-8')
        except Exception, e:
            if verbose: print 'title extract fail', str(e)
        try:
            yelpPage = r.find('a', {'class':'lemon--a__373c0__IEZFH link__373c0__29943 link-color--blue-dark__373c0__1mhJo link-size--inherit__373c0__2JXk5'})['href']
        except Exception, e:
...
def crawlGoodReviews(bizname, page_num, title, restRating, verbose=False):
	try:
		page_url = get_yelp_review_page(bizname)
	...
	for r in reviews[1:2]:
		...
		try:
			friends = r.find('li', {'class':'friend-count responsive-small-display-inline-block'}).b.getText().encode('utf-8')
		except Exception, e:
	    		if verbose: print 'friend extract fail', str(e)
		try:
			numberOfReviews = r.find('li', {'class':'review-count responsive-small-display-inline-block'}).b.getText().encode('utf-8')
		except Exception, e:
	...
	for r in filteredreviews:
		...
		try:
			authorid = r.find('span', {'class':'user-display-name'})['data-hovercard-id'].encode('utf-8')
	    		...
	    		if verbose: print 'id extract fail', str(e)
		try:
			friends = r.find('span', {'class':'friend-count responsive-small-display-inline-block'}).b.getText().encode('utf-8')
		except Exception, e:
	    		if verbose: print 'friend extract fail', str(e)
		try:
			numberOfReviews = r.find('span', {'class':'review-count responsive-small-display-inline-block'}).b.getText().encode('utf-8')
		except Exception, e:
	    		if verbose: print 'numberOfReviews extract fail', str(e)
		try:
			rating = r.find('i', {'class':re.compile(r'i-stars i-stars--')}).img['alt'].encode('utf-8')
			rating = rating[:3]
		except Exception, e:
	    		if verbose: print 'rating extract fail', str(e)
		try:
